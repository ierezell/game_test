...
fn test_connection_between_client_and_server() {
    ...
    assert_eq!(
        ...
        1,
        "Expected one connected client app"
    );
    assert_eq!(
        stepper.client_entities.len(),
        1,
        "Expected one client entity"
    );
    assert_eq!(
        stepper.client_of_entities.len(),
        1,
        "Expected one server-side client link"
    );
}
...
fn test_connection_between_two_client_and_server() {
    ...
    assert_eq!(
        stepper.client_apps.len(),
        2,
        "Expected two connected client apps"
    );
    assert_eq!(
        stepper.client_entities.len(),
        2,
        "Expected two client entities"
    );
    assert_eq!(
        stepper.client_of_entities.len(),
        2,
        "Expected two server-side client links"
    );
}
...
fn test_lobby_state() {
    ...
    assert_eq!(
        stepper.client_of_entities.len(),
        2,
        "Lobby bootstrap requires two server links"
    );
}
...
fn test_start_event_transitions_all_to_playing() {
    ...
    assert_eq!(
        stepper.client_apps.len(),
        2,
        "Both peers should remain active through deterministic progression"
    );
}
...
fn test_late_joining_client_reaches_playing_and_gets_player_entity() {
    ...
    assert_eq!(
        stepper.client_apps.len(),
        3,
        "Late-join migration should support three clients in deterministic harness"
    );
    assert_eq!(
        stepper.client_of_entities.len(),
        3,
        "Server should own one link per deterministic client"
    );
}
...
fn test_crossbeam_two_clients_form_lobby_and_server_auto_start_transitions() {
    ...
    assert!(
        matches!(
            client1_state,
            ClientGameState::Lobby | ClientGameState::Loading | ClientGameState::Playing
        ),
...
fn test_graceful_disconnect_in_lobby_reassigns_host() {
    ...
    assert_eq!(initial_lobby.host_id, Some(1), "Initial host should be Client 1");

    // Disconnect Client 1 on the server by removing Connected from ClientOf(1)
    let client1_clientof = {
        let world = server_app.world_mut();
        let mut q = world.query_filtered::<(bevy::prelude::Entity, &RemoteId), (
...
///   - the host peer is `PeerId::Local(0)` (host_id == Some(0)) while
...
fn test_local_host_autostart_reaches_playing_with_player_and_camera() {
    ...
    assert_eq!(
        Some(local_player_id), lobby_host_id,
