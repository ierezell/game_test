...
use bevy::prelude::{Color, Commands, Component, Name, PointLight, Vec2, Vec3, default, info};
...
pub fn spawn_procedural_sleepers(commands: &mut Commands, level_graph: &LevelGraph) {
    ...
    for zone in &candidate_zones {
        ...
        for i in 0..count {
            let offset = Vec3::new(
                pseudo_rand(zone.id.0 + i * 7, 0) * zone.size.x * 0.3,
                1.0,
                pseudo_rand(zone.id.0 + i * 7, 1) * zone.size.z * 0.3,
            );
...
mod tests {
    ...
    fn procedural_sleepers_have_health_and_navigation_components() {
        ...
        for (agent, health) in sleeper_query.iter(world) {
            ...
            assert!(health.current > 0.0, "Sleepers should have positive health");
        }
