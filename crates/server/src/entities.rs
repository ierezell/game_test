...
fn mark_dead_npcs_for_respawn(
    ...
    mut npc_query: Query<
        (Entity, &Health, &mut Respawnable, &mut Position, &mut LinearVelocity),
        (
...
fn respawn_dead_npcs(
    mut commands: Commands,
    time: Res<bevy::prelude::Time>,
    mut npc_query: Query<
        (Entity, &mut Health, &Respawnable, &mut Position, &mut LinearVelocity),
        (With<CharacterMarker>, Without<PlayerId>, With<PendingNpcRespawn>),
    >,
) {
    let now = time.elapsed().as_secs_f32();
