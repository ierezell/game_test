...
mod test {
    ...
    fn test_client_server_movement_sync() {
        ...
        // Send from Client 1
        let mut sender1 = client1
            .world_mut()
            .query::<&mut MessageSender<HostStartGameEvent>>()
            .single_mut(client1.world_mut())
            .expect("Client 1 sender not found");
        sender1.send::<MetadataChannel>(HostStartGameEvent);

        // Send from Client 2
        let mut sender2 = client2
            .world_mut()
            .query::<&mut MessageSender<HostStartGameEvent>>()
            .single_mut(client2.world_mut())
            .expect("Client 2 sender not found");
        ...
        if let Ok(lobby) = server_world
            .query::<&shared::protocol::LobbyState>()
            .single(server_world)
        {
        ...
        let mut client_query = server_world.query::<(
            Entity,
        ...
        for (e, name, sender, client_of) in client_query.iter(server_world) {
            ...
            println!(
                "Server Entity: {:?}, Name: {:?}, HasReplSender: {}, IsClientOf: {}",
                e, name, has_repl, is_client_of
            );
        }
        ...
        // run_apps_updates runs one update per app per cycle.
        ...
        assert!(
            distance_moved > 1.0,
            "Player should have moved > 1.0 units. Moved: {}",
            distance_moved
        );
