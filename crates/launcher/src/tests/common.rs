...
pub mod test {
    ...
    // Track if CrossbeamPlugin has been added to avoid duplicate initialization
    use std::sync::atomic::{AtomicBool, Ordering};
    static CROSSBEAM_INITIALIZED: AtomicBool = AtomicBool::new(false);

    pub fn create_test_server() -> App {
        // Set flag before creating server
        CROSSBEAM_INITIALIZED.store(true, Ordering::SeqCst);
    ...
    pub fn create_test_client(
        ...
        if auto_join {
            ...
            // Auto-join means we skip LocalMenu and go straight to Lobby
            // But we need to setup Crossbeam endpoint first
            use client::network::CrossbeamClientEndpoint;
            use lightyear::crossbeam::CrossbeamIo;
            let (client_io, _server_io) = CrossbeamIo::new_pair();
    ...
    pub fn setup_two_player_game() -> (App, App, App) {
        ...
        let mut server_query = server_app
            .world_mut()
            .query_filtered::<Entity, With<Name>>();
        let server_entity = server_query.iter(server_app.world()).find(|e| {
            server_app
                .world()
                .get::<Name>(*e)
        ...
        if let Ok(mut lobby_state) = lobby_query.single_mut(server_app.world_mut()) {
