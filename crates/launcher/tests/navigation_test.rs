mod common;
use common::{setup_two_player_game, get_spawned_npcs, get_entity_position, assert_entity_moved, run_apps_updates};
use shared::navigation::{SimpleNavigationAgent, PatrolRoute};
...
fn test_basic_navigation_functionality() {
    ...
    let has_nav_agent = server.world().get::<SimpleNavigationAgent>(npc_entity).is_some();
    ...
    let initial_pos = get_entity_position(server.world(), npc_entity)
